...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function sendTicketCompletionNotification(ticket: any) {
    const webhookUrl = process.env.GOOGLE_CHAT_WEBHOOK_URL;
    if (!webhookUrl) return;
    ...
    fetch(webhookUrl, {
        ...
        .then((res) => {
            if (!res.ok) throw new Error(`Status: ${res.status}`);
            console.log(`Completion Google Chat notification sent successfully for Ticket '${ticket.id}'`);
        })
        .catch((err) => {
            console.error(`Failed to send completion webhook for Ticket '${ticket.id}':`, err);
        });
}
